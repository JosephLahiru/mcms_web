...
function ViewLowStock() {
  ...
  const handleFilterChange = (event) => {
    ...
    if (event.target.value === "") {
      ...
      const filteredData = lowstock.filter(
        (item) => item.stock_type === Number(event.target.value)
      );
      ...
      <Grid container alignItems="center" spacing={2}>
        ...
        <Grid item xs={2}>
          <FormControl sx={{ m: 1, minWidth: 150 }} size="small">
            ...
            >
              <MenuItem value="">All low stock</MenuItem>
              <MenuItem value={1}>between 1 - 20 Qty</MenuItem>
              <MenuItem value={2}>between 21-80 Qty</MenuItem>
              <MenuItem value={3}>more than 80 Qty</MenuItem>
            </Select>
          </FormControl>
        </Grid>
        <Grid item xs={12}>
